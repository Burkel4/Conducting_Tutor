...
def generate_all_graphs(cycle_one):
    ...
    beat_plot_graph(cycle_one.processing_intervals, 
                   ...
                   cycle_one.y_peaks, cycle_one.y_valleys, 
...
def mirror_y_coordinate_graph(left_hand_y, right_hand_y):
    ...
    # plot normalized hand coordinates
    plt.plot(range(len(left_hand_y)), [y - left_hand_y[0] for y in left_hand_y], 
             label='Left Hand Y', color='r', alpha=0.7)
    plt.plot(range(len(right_hand_y)), [y - right_hand_y[0] for y in right_hand_y], 
